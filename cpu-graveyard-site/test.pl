...
$script_url = "/home/zymos/Documents/Projects/cpu_website/cpu-graveyard-site/generated_pages/chips"; #https://happytrees.org/chiptesting"; # previously $scriptName # Used for links
...
sub create_html_body_main_single_chip_table { # display table for single chip
	...
	if ( &chipData2($row,$colDataBus) ne '' ) {
		$dataW = &chipData2($row,$colDataBus);
	}
	...
	$text .= <<Endhtml;
			<tr>
				...
				<td style="white-space: nowrap; padding-left: 10px;">
					...
					<span class="tableChipValue">$dataW</span>
				</td><td style="white-space: nowrap;">
...
sub set_query_vars{
  ...
  # print "+++++++++++++++++++++++++++++++++++++++\n";
  #
	if (defined $_[0]{'page'}) { $Dpage = $_[0]{'page'}; } else { $Dpage = ''; }
	if (defined $_[0]{'name'}) { $Dname = $_[0]{'name'}; } else { $Dname = ''; }
	if (defined $_[0]{'type'}) { $Dtype = $_[0]{'type'}; } else { $Dtype = ''; }
	if (defined $_[0]{'manuf'}) { $Dmanuf = $_[0]{'manuf'}; } else { $Dmanuf = ''; }
	if (defined $_[0]{'manufacturer'}) { $Dmanufacturer = $_[0]{'manufacturer'}; } else { $Dmanufacturer = ''; }
	if (defined $_[0]{'family'}) { $Dfamily = $_[0]{'family'}; } else { $Dfamily = ''; }
	if (defined $_[0]{'cat'}) { $Dcat = $_[0]{'cat'}; } else { $Dcat = ''; }
	if (defined $_[0]{'category'}) { $Dcategory = $_[0]{'category'}; } else { $Dcategory = ''; }
	if (defined $_[0]{'part'} ) { $Dpart  = $_[0]{'part'} ; } else { $Dpart  = ''; }
  ...
  $data{'type'} = $Dtype;
  $data{'manuf'} = $Dmanuf;
  $data{'manufacturer'} = $Dmanufacturer;
  $data{'family'} = $Dfamily;
  $data{'cat'} = $Dcat;
...
sub extract_pages_content{
  ...
  foreach my $a (@p_info){ # not sure why there is a extra single surrounding array
    ...
    foreach my $page_info (@{$a}){ # array of pages
      ...
      set_query_vars($$page_info{'page_varables'});
      # %$data = ();
...
sub create_page_list{
  ...
  push @pages_to_get, { page_varables => { page => "" },
                        file_location => "$webpage_root/index.html" };
  ...
  push @pages_to_get, { page_varables =>     { page => "fullList" },
                        file_location => "$webpage_root/full_list/index.html" };
  push @pages_to_get, { page_varables =>   { page => "tradeList" },
                        file_location => "$webpage_root/trade/index.html" };
  # push @pages_to_get, { page_varables =>  { page => "cat", "category" => "manufacturers" },
  #                       file_location => "$webpage_root/manufacturers/index.html" };
  # push @pages_to_get, { page_varables =>  { page => "cat", "category" => "families" },
                        # file_location => "$webpage_root/families/index.html" };
  push @pages_to_get, { page_varables => { page => "site_map" }, 
                        file_location => "$webpage_root/sitemap/index.html" };
  push @pages_to_get, { page_varables => { page => "glossary" }, 
                        file_location => "$webpage_root/glossary/index.html" };
  ...
  push @pages_to_get, { page_varables => { page => "cat", type => "arch" },
                        file_location => "$webpage_root/architecture/index.html" };
  push @pages_to_get, { page_varables => { page => "cat", type => "data_width" },
                        file_location => "$webpage_root/data_width/index.html" };
  push @pages_to_get, { page_varables => { page => "cat", type => "type" },
                        file_location => "$webpage_root/chip_types/index.html" };
  push @pages_to_get, { page_varables => { page => "cat", type => "isa" },
                        file_location => "$webpage_root/ISA/index.html" };
  push @pages_to_get, { page_varables => { page => "cat", type => "tech" },
                        file_location => "$webpage_root/technologies/index.html" };
  push @pages_to_get, { page_varables => { page => "cat", type => "date" },
                        file_location => "$webpage_root/release_date/index.html" };
  push @pages_to_get, { page_varables => { page => "cat", type => "logo" },
                        file_location => "$webpage_root/logos/index.html" };
  ...
  #                       file_location => "$webpage_root/mf/$manuf/$fam/index.html" };
  #     # print " --> $manuf - $fam\n";
  #   }
  # }
  #
